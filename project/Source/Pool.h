...
public:
    ...
    void Free(T* pValue)
    {
        ...
        if (item->m_pPrev != nullptr)
            item->m_pPrev->m_pNext = item->m_pNext;

        if (item->m_pNext != nullptr)
    ...
    void ShaderVisible() { m_IsShaderVisible = true; }
    void NoShaderVisible() { m_IsShaderVisible = false; }
...
private:
    ...
    bool        m_IsShaderVisible;  //!< シェーダ可視フラグです.
