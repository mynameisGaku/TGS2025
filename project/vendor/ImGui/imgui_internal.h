...
// Extend ImGuiRootNodeFlags_
enum ImGuiRootNodeFlagsPrivate_
{
    ImGuiRootNodeFlags_ClipLabelForTrailingButton = 1 << 20,
    ImGuiRootNodeFlags_UpsideDownArrow            = 1 << 21,// (FIXME-WIP) Turn Down arrow into an Up arrow, but reversed trees (#6517)
};
...
// Store data emitted by TreeNode() for usage by TreePop() to implement ImGuiRootNodeFlags_NavLeftJumpsBackHere.
...
//#endif
    ...
    IMGUI_API bool          TreeNodeBehavior(ImGuiID id, ImGuiRootNodeFlags flags, const char* label, const char* label_end = NULL);
    IMGUI_API void          TreePushOverrideID(ImGuiID id);
    IMGUI_API void          TreeNodeSetOpen(ImGuiID id, bool open);
    IMGUI_API bool          TreeNodeUpdateNextOpen(ImGuiID id, ImGuiRootNodeFlags flags);   // Return open state. Consume previous SetNextItemOpen() data, if any. May return true when logging.
    ...
    inline bool     TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiRootNodeFlags flags = 0)    { return TreeNodeUpdateNextOpen(id, flags); }   // Renamed in 1.89
