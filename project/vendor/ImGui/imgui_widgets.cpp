...
bool ImGui::TreeNodeEx(const char* label, ImGuiRootNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::TreeNodeEx(const char* str_id, ImGuiRootNodeFlags flags, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
bool ImGui::TreeNodeEx(const void* ptr_id, ImGuiRootNodeFlags flags, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
bool ImGui::TreeNodeExV(const char* str_id, ImGuiRootNodeFlags flags, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::TreeNodeExV(const void* ptr_id, ImGuiRootNodeFlags flags, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiRootNodeFlags flags)
{
    if (flags & ImGuiRootNodeFlags_Leaf)
        return true;
    ...
    {
        is_open = storage->GetInt(id, (flags & ImGuiRootNodeFlags_DefaultOpen) ? 1 : 0) != 0;
    }

    // When logging is enabled, we automatically expand tree nodes (but *NOT* collapsing headers.. seems like sensible behavior).
    // NB- If we are above max depth we still allow manually opened nodes to be logged.
    if (g.LogEnabled && !(flags & ImGuiRootNodeFlags_NoAutoOpenOnLog) && (window->DC.TreeDepth - g.LogDepthRef) < g.LogDepthToExpand)
        is_open = true;

    return is_open;
}

bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiRootNodeFlags flags, const char* label, const char* label_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const bool display_frame = (flags & ImGuiRootNodeFlags_Framed) != 0;
    const ImVec2 padding = (display_frame || (flags & ImGuiRootNodeFlags_FramePadding)) ? style.FramePadding : ImVec2(style.FramePadding.x, ImMin(window->DC.CurrLineTextBaseOffset, style.FramePadding.y));
    ...
    const bool span_all_columns = (flags & ImGuiRootNodeFlags_SpanAllColumns) != 0 && (g.CurrentTable != NULL);
    ImRect frame_bb;
    frame_bb.Min.x = span_all_columns ? window->ParentWorkRect.Min.x : (flags & ImGuiRootNodeFlags_SpanFullWidth) ? window->WorkRect.Min.x : window->DC.CursorPos.x;
    ...
    if (!display_frame && (flags & (ImGuiRootNodeFlags_SpanAvailWidth | ImGuiRootNodeFlags_SpanFullWidth | ImGuiRootNodeFlags_SpanAllColumns)) == 0)
    ...
    // If a NavLeft request is happening and ImGuiRootNodeFlags_NavLeftJumpsBackHere enabled:
    // Store data for the current depth to allow returning to this node from any child item.
    // For this purpose we essentially compare if g.NavIdIsAlive went from 0 to 1 between TreeNode() and TreePop().
    // It will become tempting to enable ImGuiRootNodeFlags_NavLeftJumpsBackHere by default or move it to ImGuiStyle.
    // Currently only supports 32 level deep and we are fine with (1 << Depth) overflowing into a zero, easy to increase.
    if (is_open && !g.NavIdIsAlive && (flags & ImGuiRootNodeFlags_NavLeftJumpsBackHere) && !(flags & ImGuiRootNodeFlags_NoTreePushOnOpen))
    ...
    const bool is_leaf = (flags & ImGuiRootNodeFlags_Leaf) != 0;
    if (!item_add)
    {
        if (is_open && !(flags & ImGuiRootNodeFlags_NoTreePushOnOpen))
    ...
    ImGuiButtonFlags button_flags = ImGuiRootNodeFlags_None;
    if ((flags & ImGuiRootNodeFlags_AllowOverlap) || (g.LastItemData.InFlags & ImGuiItemFlags_AllowOverlap))
        button_flags |= ImGuiButtonFlags_AllowOverlap;
    if (!is_leaf)
    ...
    else if (flags & ImGuiRootNodeFlags_OpenOnDoubleClick)
        button_flags |= ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnDoubleClick;
    else
        button_flags |= ImGuiButtonFlags_PressedOnClickRelease;

    bool selected = (flags & ImGuiRootNodeFlags_Selected) != 0;
    ...
    if (!is_leaf)
    {
        ...
        {
            if ((flags & (ImGuiRootNodeFlags_OpenOnArrow | ImGuiRootNodeFlags_OpenOnDoubleClick)) == 0 || (g.NavActivateId == id))
                toggled = true;
            if (flags & ImGuiRootNodeFlags_OpenOnArrow)
                toggled |= is_mouse_x_over_arrow && !g.NavDisableMouseHover; // Lightweight equivalent of IsMouseHoveringRect() since ButtonBehavior() already did the job
            if ((flags & ImGuiRootNodeFlags_OpenOnDoubleClick) && g.IO.MouseClickedCount[0] == 2)
                toggled = true;
        }
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiRootNodeFlags_Bullet)
            RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.60f, text_pos.y + g.FontSize * 0.5f), text_col);
        else if (!is_leaf)
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ((flags & ImGuiRootNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 1.0f);
        else // Leaf without bullet, left-adjusted text
            text_pos.x -= text_offset_x -padding.x;
        if (flags & ImGuiRootNodeFlags_ClipLabelForTrailingButton)
    ...
    else
    {
        ...
        if (flags & ImGuiRootNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ((flags & ImGuiRootNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0.70f);
        if (g.LogEnabled)
            LogSetNextTextDecoration(">", NULL);
    }
...
void ImGui::TreePop()
{
    ...
    // Handle Left arrow to move to parent tree node (when ImGuiRootNodeFlags_NavLeftJumpsBackHere is enabled)
...
// CollapsingHeader returns true when opened but do not indent nor push into the ID stack (because of the ImGuiRootNodeFlags_NoTreePushOnOpen flag).
// This is basically the same as calling TreeNodeEx(label, ImGuiRootNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen flag if you want behavior closer to normal TreeNode().
bool ImGui::CollapsingHeader(const char* label, ImGuiRootNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeNodeBehavior(window->GetID(label), flags | ImGuiRootNodeFlags_CollapsingHeader, label);
}

// p_visible == NULL                        : regular collapsing header
// p_visible != NULL && *p_visible == true  : show a small close button on the corner of the header, clicking the button will set *p_visible = false
// p_visible != NULL && *p_visible == false : do not show the header at all
// Do not mistake this with the Open state of the header itself, which you can adjust with SetNextItemOpen() or ImGuiRootNodeFlags_DefaultOpen.
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiRootNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    flags |= ImGuiRootNodeFlags_CollapsingHeader;
    if (p_visible)
        flags |= ImGuiRootNodeFlags_AllowOverlap | (ImGuiRootNodeFlags)ImGuiRootNodeFlags_ClipLabelForTrailingButton;
