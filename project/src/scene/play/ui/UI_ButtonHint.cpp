...
namespace
{
	static const float HEIGHT = 40.0f;	// 表示領域の高さ
	static const RGBColor BACK_COLOR = RGBColor(100);	// 背景色
}
...
void UI_ButtonHint::drawBack()
{
	...
	DrawBox(pos.x, pos.y + size.y - HEIGHT, pos.x + size.x, pos.y + size.y, BACK_COLOR.GetColorInt(), true); // 背景を描画
}
